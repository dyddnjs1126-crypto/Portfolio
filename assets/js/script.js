    ...
    function setModalGalleryMode(type, totalImages) {
      // 이미지가 2장 이상이면 카드뉴스뿐 아니라 배너/포스터/릴스에서도 썸네일 갤러리를 사용합니다.
      const hasGallery = totalImages > 1;

      modalThumbs.classList.toggle('is-hidden', !hasGallery);
      modalPrev.classList.toggle('is-hidden', !hasGallery);
      modalNext.classList.toggle('is-hidden', !hasGallery);
      modalPageIndicator.classList.toggle('is-hidden', !hasGallery);
    }
    ...
    function openModal(source) {
      ...
      // data-modal-images="이미지1|이미지2|이미지3" 형식으로 여러 이미지를 넣을 수 있습니다.
      // data-modal-images가 없을 때만 data-modal-media 또는 카드 안의 대표 미디어를 사용합니다.
